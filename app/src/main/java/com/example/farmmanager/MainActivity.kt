...
class MainActivity : AppCompatActivity() {
    ...
    //전역변수
    companion object{
        var humity_in = 40   // jw: 앱 안에서 설정한 값 _in
        var humity_out = 80  // jw: 실제 값(센서가 측정한 값) _out
        var water_in = 40
        var water_out = 80
        var alarm = false
    }
...
class SocketReceiver : Thread() {  // jw: :Thread() <- 스레드라는 클래스를 상속한다.
...
class SocketClient : Serializable {                     //jw: Serializable(직렬화) = 데이터 스트림을 사용 할 때
    companion object{
        val serverIP = "192.168.0.2" // 서버의 IP           //jw: 의천이형 노트북 ip ㄸㄹㅅ, 라즈베리파이 ip 로 나중에 바꿔줘야함
        val port = 3022 // 서버의 port                      //jw: 데이터를 받아 낼 포트( 사용하지 않는 포트를 랜덤으로 설정 한 것 )


        lateinit var originSocket: Socket                   //jw: (tip) lateinit = 지연초기화 : 변수를 만들어 놓고 나중에 값을 할당 하겠다
        ...
        var socketBtn = false                               //jw: 연결 여부를 확인하는 변수 [ true: 연결/ false: 연결되지 않음 ]
